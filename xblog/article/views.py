...
def article_column(request):
    ...
    if request.method == "POST":
        ...
        column_form = ArticleColumn.objects.filter(user_id=request.user.id, column=column_name)
        if columns:
            return HttpResponse('2')
        else:
            ArticleColumn.objects.creat(user=request.user, column=column_name)
...
def article_post(request):
    if request.method == "POST":
        ...
        article_post_form = ArticleColumnForm()
...
def article_tag(request):
    ...
    if request.method == "POST":
        tag_post_form = ArticleTag(data=request.POST)
        if tag_post_form.is_valid():
            ...
            return HttpResponse("sorry, the data can not be save")


@login_required(login_url='/account/login/')
@require_POST
@csrf_exempt
def def_article_tag(request):
